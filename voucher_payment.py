...
class PaymentVoucher:
    def __init__(self, parent, username, working_date):
        ...
        self.window.title("Payment Voucher")
    ...
    def generate_voucher_no(self):
        ...
        c.execute("SELECT COUNT(*) FROM vouchers WHERE voucher_no LIKE ?", (f'PMT-{today}%',))
        count = c.fetchone()[0] + 1
        conn.close()
        self.voucher_no = f"PMT-{today}-{count:03d}"
        self.voucher_no_var.set(self.voucher_no)
    
    def load_all_accounts(self):
        """Load ALL accounts where payments can be made (Liabilities, Expenses)"""
        try:
            conn = sqlite3.connect('urban_pulse.db')
            c = conn.cursor()
            
            # Load Liability, Expense accounts, and Supplier accounts
            c.execute('''SELECT id, account_code, account_name, account_type, current_balance 
                         FROM accounts 
                         WHERE account_type IN ('Liability', 'Expense')
                         OR account_code LIKE '2-%'  -- Supplier accounts under Liability
                         OR account_code IN ('2000', '2-2100', '2-2200')
                         ORDER BY 
                           CASE 
                             WHEN account_code LIKE '2-%' THEN 1  -- Suppliers first
            ...
            if self.all_accounts:
                ...
                for acc in self.all_accounts:
                    ...
                    # Add balance with Dr/Cr indicator - CRITICAL FIX: Show balance like receipt voucher
                    if balance != 0:
                        if balance > 0:
                            ...
                            if acc_type in ['Asset', 'Expense']:
                                balance_display = f"Cr {CURRENCY_SYMBOL}{abs_balance:,.0f}"
                            else:
                                balance_display = f"Dr {CURRENCY_SYMBOL}{abs_balance:,.0f}"
                        display_text += f" ({balance_display})"
                    
                    self.all_accounts_display.append(display_text)
                
                print(f"✅ Loaded {len(self.all_accounts)} accounts for payment")
            else:
    ...
    def update_balance_display(self):
        """Update the balance display with selected account info - FIXED to show balance properly"""
        try:
            if self.selected_code and self.selected_code.startswith('2-'):
                prefix = "🏭 Supplier Account"
            elif self.selected_code == '2000':
                prefix = "📋 Accounts Payable"
            elif self.selected_type == 'Liability':
                prefix = "📋 Liability Account"
            elif self.selected_type == 'Expense':
                prefix = "📉 Expense Account"
            else:
                prefix = "📊 Account"
            
            # Format balance with Dr/Cr - EXACTLY like receipt voucher
    ...
    def show_account_popup(self, entry_widget):
        """Show popup with filtered accounts - FIXED to show balances"""
    ...
    def create_widgets(self):
        ...
        tk.Label(title_frame, text="💸 PAYMENT VOUCHER", fg='white', bg='#e74c3c',
        ...
        type_frame = tk.LabelFrame(main_frame, text="Payment Type", font=('Arial', 11, 'bold'),
                                  padx=10, pady=10)
        type_frame.pack(fill=tk.X, pady=10)
        
        self.payment_type = tk.StringVar(value="payable")
        
        tk.Radiobutton(type_frame, text="📋 Accounts Payable (Supplier)", variable=self.payment_type, 
                      value="payable", bg='#f9f9f9', font=('Arial', 10)).grid(row=0, column=0, padx=5, pady=5, sticky='w')
        tk.Radiobutton(type_frame, text="📉 Direct Expense", variable=self.payment_type, 
                      value="expense", bg='#f9f9f9', font=('Arial', 10)).grid(row=1, column=0, padx=5, pady=5, sticky='w')
        ...
        self.account_label = tk.Label(account_frame, text="Select Account:", font=('Arial', 10), bg='#f9f9f9')
        ...
        # Balance display - FIXED to show properly
        ...
        # Payment Details
        payment_frame = tk.LabelFrame(main_frame, text="Payment Details", font=('Arial', 11, 'bold'),
                                     padx=10, pady=10)
        payment_frame.pack(fill=tk.X, pady=10)
        
        tk.Label(payment_frame, text="Amount:").grid(row=0, column=0, padx=5, pady=5, sticky='e')
        self.amount = tk.Entry(payment_frame, width=20, font=('Arial', 10), relief='solid', bd=1)
        self.amount.grid(row=0, column=1, padx=5, pady=5, sticky='w')
        tk.Label(payment_frame, text=CURRENCY_SYMBOL, font=('Arial', 10)).grid(row=0, column=2, padx=5, pady=5, sticky='w')
        
        tk.Label(payment_frame, text="Payment Mode:").grid(row=1, column=0, padx=5, pady=5, sticky='e')
        self.payment_mode = ttk.Combobox(payment_frame, values=['Cash', 'Cheque', 'Bank Transfer', 'Card'], 
                                        width=17, font=('Arial', 10))
        self.payment_mode.set('Cash')
        self.payment_mode.grid(row=1, column=1, padx=5, pady=5, sticky='w')
        
        tk.Label(payment_frame, text="Reference No:").grid(row=2, column=0, padx=5, pady=5, sticky='e')
        self.reference = tk.Entry(payment_frame, width=30, font=('Arial', 10), relief='solid', bd=1)
    ...
    def save_voucher(self):
        """Save payment voucher with correct accounting"""
        ...
        try:
            ...
            c.execute('''INSERT INTO vouchers 
                        ...
                        VALUES (?,?,?,?,?,?,?)''',
                      (self.voucher_no, 'Payment', self.working_date, 
                       f"{narration} | Ref: {reference} | Mode: {payment_mode}", 
                       amount, self.username, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
            voucher_id = c.lastrowid
            
            # Get Cash/Bank account ID based on payment mode
            ...
            # Credit Cash/Bank (payment goes out)
            c.execute('''INSERT INTO ledger_entries 
                        (voucher_id, account_id, debit, credit, entry_date, narration)
                        VALUES (?,?,?,?,?,?)''',
                      (voucher_id, cash_id, 0, amount, self.working_date, 
                       f"Payment to {self.selected_name}"))
            ...
            c.execute('''INSERT INTO ledger_entries 
                        (voucher_id, account_id, debit, credit, entry_date, narration)
                        VALUES (?,?,?,?,?,?)''',
                      (voucher_id, selected_account_id, amount, 0, self.working_date, 
                       f"Payment debit"))
            
            # Update selected account balance based on account type
            if self.selected_type == 'Liability':
                # For liability accounts (like Payable), debit decreases balance
                c.execute("UPDATE accounts SET current_balance = current_balance - ? WHERE id=?", 
            ...
            # Update balance label with new balance showing Dr/Cr - FIXED to show properly
            if new_balance > 0:
                if self.selected_type in ['Asset', 'Expense']:
